...
Status CsvScanNode::Init(TPlanNode& tnode){
...
Status CsvScanNode::Prepare(RuntimeState* state) {
        LOG(ERROR) << "We entered Prepare func";
...
Status CsvScanNode::Open(RuntimeState* state) {
  ...
  table_file = new ifstream(filename.c_str());
  if(!(*table_file))LOG(ERROR) << "table file cannot be opened";
  LOG(ERROR) << "we returned from Open";
	return Status::OK;
}
...
//This function not only allocates memory for tuples in a batch
//but also initialize all rows so no more operation is needed in
//NOTE: rowbatch has safety check cannot init all 
//future. It has an overhead of initializing unused rows though.
Tuple* CsvScanNode::AllocateBatchTupleMem(RowBatch* row_batch){
  uint8_t* retVal = row_batch->tuple_data_pool()->Allocate(row_batch->capacity() * tuple_desc_->byte_size());
  memset(retVal, 0, sizeof(uint8_t) * row_batch->capacity() * tuple_desc_->byte_size());
  return reinterpret_cast<Tuple *> (retVal);
}

Status CsvScanNode::ParseAndFill(std::string &data, RowBatch* row_batch){
  int row_no = row_batch->AddRow();
  TupleRow* row = row_batch->GetRow(row_no);
  //init row
  row -> SetTuple(0, reinterpret_cast<Tuple *>(tuple_mem_ + row_no * tuple_desc_->byte_size()));
  ...
  for (size_t i = 0; i < materialized_slots_.size(); ++i) {
    //Safety check for missing data
    int len;
    char *field;
    if( !(materialized_slots_[i]->col_pos()<tokens.size())){
      len = 0;
      field = NULL;
    }
    else{
      len = tokens[materialized_slots_[i]->col_pos()].size();
      field = (char *)tokens[materialized_slots_[i]->col_pos()].c_str();
    }
    SlotDescriptor* desc = materialized_slots_[i];
    Tuple * tuple = row->GetTuple(0);
    MemPool* pool = row_batch->tuple_data_pool();
    LOG(ERROR) << "Try to write " << tokens[materialized_slots_[i]->col_pos()] << " into slot " << materialized_slots_[i]->col_pos();
    bool ok = WriteSlot(desc, tuple, field, len, pool);
    //if(!ok){
    //  Status status = Status::CANCELLED;
    //  status.SetErrorMsg(ErrorMsg(TErrorCode::GENERAL, "CSV_SCAN_NODE failed to parse an item while reading input file"));
    //  return status;
    //}
  }
  ExprContext* const* conjunct_ctxs = &conjunct_ctxs_[0];
  size_t num_join_ctxs = conjunct_ctxs_.size();
  if(EvalConjuncts(conjunct_ctxs, num_join_ctxs, row)){
...
Status CsvScanNode::GetNextInternal(RuntimeState* state, RowBatch* row_batch,
	...
	tuple_ = AllocateBatchTupleMem(row_batch);
	tuple_mem_ = reinterpret_cast<uint8_t *>(tuple_);
	...
	while(1){
	  ...
	  std::string line;
	  //read reach eof
	  if(!std::getline(*table_file, line)){
	    *eos = true;
	    break;
	  }
	  //parse and fill
	  LOG(ERROR) << "We read a new line : " << line;
	  RETURN_IF_ERROR(ParseAndFill(line, row_batch));
	  //limit reached?
	  if (ReachedLimit()){
	    *eos = true;
	    break;
