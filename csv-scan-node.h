...
class CsvScanNode : public ExecNode {
 public:
  ...
  //init
  virtual Status Init(TPlanNode& tnode);
  //parse and fill
  Status ParseAndFill(std::string& s, RowBatch *row_batch);
  //babysit memory allocations
  Tuple * AllocateBatchTupleMem(RowBatch *row_batch);
